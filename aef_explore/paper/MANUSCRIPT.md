...
Event tables, the reconstructed model, the figure scripts and the numbers trace accompany this
manuscript. The embedding, imagery, alert and forest products are all public and are named in
Section 2. The Congo re-dating uses `wur_radd_alerts` version `v20220109` from the Global
Forest Watch Data API, which requires a free API key.
