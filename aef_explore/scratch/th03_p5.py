...
def sweep(st,tag,nlat,nlon):
    ...
    for i in range(nlat):
        for j in range(nlon):
            ...
            try: rows+=grab(st,ee.Geometry.Rectangle([x0,y0,x0+dx,y0+dy]))
            except Exception as ex:
                for k in range(4):
                    bb=ee.Geometry.Rectangle([x0+(k%2)*dx/2,y0+(k//2)*dy/2,x0+(k%2+1)*dx/2,y0+(k//2+1)*dy/2])
                    try: rows+=grab(st,bb)
                    except Exception as e2: print(tag,i,j,k,'FAIL',str(e2)[:50],flush=True)
        print(tag,'lat',i,'cum',len(rows),flush=True)
